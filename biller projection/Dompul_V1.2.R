...
KrakenBillerProductAxiataDompulHD2<-readRDS(file="KrakenBillerProductAxiataDompulHD2.Rds")
KrakenBillerProductAxiataDompulD2<-readRDS(file="KrakenBillerProductAxiataDompulD2.Rds")

KrakenBillerProductAxiataDompulH<-rbind(KrakenBillerProductAxiataDompulHD50,KrakenBillerProductAxiataDompulHD2)
KrakenBillerProductAxiataDompul<-rbind(KrakenBillerProductAxiataDompulD50,KrakenBillerProductAxiataDompulD2)
...
Axiata7214F<-forecast(Axiata7214SMA2,h=5)
Axiata7215F<-forecast(Axiata7215SMA2,h=5)
Axiata7217F<-forecast(Axiata7217SMA2,h=5)
Axiata7222F<-forecast(Axiata7222SMA2,h=5)
Axiata9531F<-forecast(Axiata9531SMA2,h=5)
Axiata9572F<-forecast(Axiata9572SMA2,h=5)
Axiata9575F<-forecast(Axiata9575SMA2,h=5)
Axiata9578F<-forecast(Axiata9578SMA2,h=5)
Axiata9579F<-forecast(Axiata9579SMA2,h=5)
#Axiata9580F<-forecast(Axiata9580SMA2,h=3)
Axiata9631F<-forecast(Axiata9631SMA2,h=5)
Axiata9671F<-forecast(Axiata9671SMA2,h=5)
Axiata9676F<-forecast(Axiata9676SMA2,h=5)
Axiata9710F<-forecast(Axiata9710SMA2,h=5)
Axiata9712F<-forecast(Axiata9712SMA2,h=5)
Axiata9713F<-forecast(Axiata9713SMA2,h=5)
Axiata9714F<-forecast(Axiata9714SMA2,h=5)
Axiata9748F<-forecast(Axiata9748SMA2,h=5)
Axiata9754F<-forecast(Axiata9754SMA2,h=5)
Axiata9757F<-forecast(Axiata9757SMA2,h=5)
Axiata9760F<-forecast(Axiata9760SMA2,h=5)
Axiata9775F<-forecast(Axiata9775SMA2,h=5)
#Axiata9776F<-forecast(Axiata9776SMA2,h=5)
Axiata9777F<-forecast(Axiata9777SMA2,h=5)
Axiata9995F<-forecast(Axiata9995SMA2,h=5)
...
AxiataADFt<-AxiataADFt[,c(1,7,2:6)]


#7. high level forecast

KrakenBillerProductAxiataDompulH$trxValue[KrakenBillerProductAxiataDompulH$trxValue<mean(KrakenBillerProductAxiataDompulH[c(44:50),2])| KrakenBillerProductAxiataDompulH$trxValue>mean(KrakenBillerProductAxiataDompulH[c(44:50),2])]<-mean(KrakenBillerProductAxiataDompulH[c(44:50),2])*1.07
...
AxiataHF<-forecast(AxiataHSMA2,h=5)
...
AxiataADFt$M1<-round((AxiataADFt$V1/100)*AxiataHDF[1,],0)
AxiataADFt$M2<-round((AxiataADFt$V2/100)*AxiataHDF[2,],0)
AxiataADFt$M3<-round((AxiataADFt$V3/100)*AxiataHDF[3,],0)
AxiataADFt$M4<-round((AxiataADFt$V4/100)*AxiataHDF[4,],0)
AxiataADFt$M5<-round((AxiataADFt$V5/100)*AxiataHDF[5,],0)

AxiataADFt<-AxiataADFt[,c(1:2,8:12)]

# Add projection date in table

currentDate<-as.Date(Sys.Date())
currentDate
Future1<-as.character(currentDate)
Future2<-as.character(currentDate+1)
Future3<-as.character(currentDate+2)
Future4<-as.character(currentDate+3)
Future5<-as.character(currentDate+4)


# Add date to projection table

colnames(AxiataADFt)[3:7]<-c(Future1,Future2,Future3,Future4,Future5)


#Deviation
# dataset for deviation (last 4 period)
...
#form projection dataset, add date & formatting
ProSch<-AxiataADFt
ProSch1<-as.data.frame(cbind(ProSch[c(1:23),3],"date"=Future1))
ProSch2<-as.data.frame(cbind(ProSch[c(1:23),4],"date"=Future2))
ProSch3<-as.data.frame(cbind(ProSch[c(1:23),5],"date"=Future3))
ProSch4<-as.data.frame(cbind(ProSch[c(1:23),6],"date"=Future4))
ProSch5<-as.data.frame(cbind(ProSch[c(1:23),7],"date"=Future5))
...
ProSch<-rbind(ProSch1,ProSch2,ProSch3,ProSch4,ProSch5)
ProSch<-merge(x=ProSch,y=dompulCode[c(1:3)],by="dompul code",all.x=TRUE)

#rearrange the schema
ProSch<-ProSch[,c(4,2:3,1,5)]
#merge with stdev data
#ProSch<-merge(x=ProSch,y=StCvAxiata,by="dompul code")
#calculate net after stdev calculated
#ProSch$deposit<-round(ProSch$deposit+(ProSch$StDev*5/4)/5,0)
#ProSch<-ProSch[,c(1:5)]

ratio<-sum(ProSch$deposit)/sum(KrakenBillerProductAxiataDompulHo[c(46:50),2])
ratio


#Create net deposit table
#Obtain deposit data
...
#insert id to projection datasbase
...
AxiataProj<-AxiataProj[c(24,13,8,14,9,15,11,16,17,10,18,1,12,20,19,7,2,6,5,23,22,3,4,21),]
